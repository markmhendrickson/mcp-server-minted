...
async def call_tool(name: str, arguments: Any) -> List[TextContent]:
    ...
    if name == "get_minted_contacts":
        ...
        try:
            ...
            try:
                ...
                if groups_response.status_code == 200:
                    ...
                    if isinstance(groups_data, list) and len(groups_data) > 0:
                        ...
                        order_groups = [
                            g for g in groups_data if g.get("type") == "completeorder"
                        ]
                        if order_groups:
                        ...
                        if response.status_code == 200:
                            ...
                            if isinstance(data, list) and len(data) > 0:
                                ...
                                order_id = (
                                    ...
                                    or latest_order.get("order_number")
                                    or latest_order.get("orderId")
                                )
                                if order_id:
                                    detail_endpoints = [
            ...
            if not delivery_data:
                try:
                    ...
                    try:
                        ...
                        login_submit = driver.find_element(
                            By.XPATH,
                            '//*[@id="__next"]/div[3]/div/form/div[2]/div[1]/button',
                        )
                        login_submit.click()
                        sleep(5)
                            ...
                            try:
                                ...
                                if json_match:
                                    try:
                                        ...
                                        if (
                                            "orders" in page_data
                                            or "deliveries" in page_data
                                            or "finalize" in page_data
                                        ):
                                            delivery_data = page_data
                                            working_endpoint = (
                                                f"scraped_from:{finalize_url}"
                                            )
                                            break
                                    except:
                                        pass
                                ...
                                try:
                                    recipient_elements = driver.find_elements(
                                        By.CSS_SELECTOR,
                                        "[data-recipient], .recipient, [class*='recipient'], [class*='address']",
                                    )
            ...
            if not delivery_data:
                endpoint_list = [
                    e[0] if isinstance(e, tuple) else e for e in endpoints_to_try
                ]
                return [
                    TextContent(
                        type="text",
            ...
            if "recipients" in latest and latest["recipients"]:
                recipients = (
                    latest["recipients"]
                    if isinstance(latest["recipients"], list)
                    else [latest["recipients"]]
                )
            # Otherwise try other field names
            elif "addresses" in latest:
                recipients = (
                    latest["addresses"]
                    if isinstance(latest["addresses"], list)
                    else [latest["addresses"]]
                )
            elif "contacts" in latest:
                recipients = (
                    latest["contacts"]
                    if isinstance(latest["contacts"], list)
                    else [latest["contacts"]]
                )
            elif "recipient_addresses" in latest:
                recipients = (
                    latest["recipient_addresses"]
                    if isinstance(latest["recipient_addresses"], list)
                    else [latest["recipient_addresses"]]
                )
            elif "shipping_addresses" in latest:
                recipients = (
                    latest["shipping_addresses"]
                    if isinstance(latest["shipping_addresses"], list)
                    else [latest["shipping_addresses"]]
                )
                    ...
                    text=json.dumps(
                        {
                            ...
                            "delivery_date": latest.get(
                                "created_at",
                                latest.get("date", latest.get("order_date", "Unknown")),
                            ),
                            "order_id": latest.get(
                                "id",
                                latest.get(
                                    "order_id", latest.get("order_number", "Unknown")
                                ),
                            ),
